...
export default function SyncHttpKernel() {
  ...
  // Read existing kernel to preserve middleware and middlewareGroups
  ...
  const middlewareFiles = fs.readdirSync(MiddlewarePath).filter(f => f.endsWith(".ts"));
  
  const controllerImports = controllerFiles.map(f => {
    const name = f.replace(".ts", "");
    return `import ${name} from "./Controllers/${f}";`;
  }).join("\n");
  
  const middlewareImports = middlewareFiles.map(f => {
    const name = f.replace(".ts", "");
    return `import ${name} from "./Middleware/${f}";`;
  }).join("\n");
  
  const routeMiddleware = middlewareFiles.map(f => {
    const name = f.replace(".ts", "");
    const key = name.replace("Middleware", "");
    return `${key}: new ${name}()`;
  }).join(",\n    ");
  
  const controllers = controllerFiles.map(f => {
    const name = f.replace(".ts", "");
